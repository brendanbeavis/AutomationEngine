...
namespace AutomationEngine.Infrastructure.Persistence.Repositories
{
    /// <summary>
    ...
    public class JobRepository : IJobRepository
    {
        ...
        public async Task DeleteJobAsync(string jobId)
        {
            try
            {
                ...
                var job = await context.Jobs.FirstOrDefaultAsync(j => j.JobId == jobId && j.DeletedAt == null);
                if (job is not null)
                {
                    ...
                    _logger.LogInformation("Job soft-deleted in repository | JobId: {JobId} | MarkedRunsCount: {Count}",
                        jobId, runs.Count);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to soft-delete job in repository | JobId: {JobId}", jobId);
                throw;
            }
        }

        public async Task HardDeleteJobAsync(string jobId)
        {
            try
            {
                using var context = _contextFactory.CreateDbContext();
            ...
            {
                _logger.LogError(ex, "Failed to hard-delete job in repository | JobId: {JobId}", jobId);
                throw;
            }
        }
